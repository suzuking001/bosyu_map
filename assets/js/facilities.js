...
function addFacilitiesFromDataset(facilityMap, fac, source) {
  ...
  const facApplyDateIndex = facHeaders.indexOf("申込数集計日");
  const recruitColumns = {};
  const applyColumns = {};
  facHeaders.forEach((header, index) => {
    let match = header.match(/^募集人数（(\d+)歳/);
    if (match) {
      recruitColumns[Number(match[1])] = index;
      return;
    }
    match = header.match(/^申込人数（(\d+)歳/);
    if (match) {
      applyColumns[Number(match[1])] = index;
    }
  });
  const recruitAges = Array.from(
    new Set([
      ...Object.keys(recruitColumns).map(Number),
      ...Object.keys(applyColumns).map(Number),
    ])
  ).sort((a, b) => a - b);
  ...
  facRows.forEach(row => {
    ...
    const recruitment = recruitAges.map(age => ({
      age,
      recruit: recruitColumns[age] != null ? row[recruitColumns[age]] : "",
      apply: applyColumns[age] != null ? row[applyColumns[age]] : "",
    }));
    ...
    facilityMap[noKey] = {
      ...
      applyDate: facApplyDateIndex >= 0 ? row[facApplyDateIndex] : "",
      recruitment,
      source,
    };
  });
